...
class ScheduleSubscriptionScreen extends StatefulWidget {
  ...
  const ScheduleSubscriptionScreen({super.key, required this.subscription});

  @override
  State<ScheduleSubscriptionScreen> createState() => _ScheduleSubscriptionScreenState();
}

class _ScheduleSubscriptionScreenState extends State<ScheduleSubscriptionScreen> {
  ...
  void initState() {
    ...
    // Ensure minDate is at least tomorrow
  ...
  void _onDaySelected(DateTime selectedDay, DateTime focusedDay) {
    if (selectedDay.isBefore(_minDate) || selectedDay.isAfter(_maxDate)) return;
      ...
      if (_selectedDays.contains(day)) {
        ...
        if (_selectedDays.length >= _remaining) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('You can only select up to $_remaining dates'))
          );
          return;
        }
  ...
  Future<void> _submit() async {
    ...
    try {
      final dates = _selectedDays.map((d) => DateFormat('YYYY-MM-DD').format(d)).toList();
      await context.read<ApiService>().selectSubscriptionDates(widget.subscription['id'], dates);
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Visits scheduled successfully!'))
        );
                ...
                children: [
                  ...
                  if (_selectedDays.isNotEmpty) ...[
                    const Align(
                      ...
                      child: Text('Selected Dates', style: TextStyle(fontWeight: FontWeight.bold)),
                    ),
                    ...
                    Wrap(
                      ...
                      children: _selectedDays.map((d) => Chip(
                        ...
                        onDeleted: () => setState(() => _selectedDays.remove(d)),
