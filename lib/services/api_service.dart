...
class ApiService {
  static const String baseUrl = 'https://gkm.gobt.in/api'; // no trailing slash
  ...
  Future<Map<String, dynamic>> _multipart(
      ...
      {File? file, String fieldName = 'file'}) async {
    final req =
  ...
  /// verifyOtp now also sends fcm_token so backend registers for notifications
  ...
  Future<Map<String, dynamic>> updateProfileWithImage(
          Map<String, dynamic> data, File image) =>
      _multipart('PUT', '/auth/profile',
          data.map((k, v) => MapEntry(k, v?.toString() ?? '')),
          file: image, fieldName: 'profile_image');

  // ── Plans, Zones & Geofences ──────────────────────────────────────────────
  Future<Map<String, dynamic>> getPlans() => get('/plans');
  Future<Map<String, dynamic>> getZones() => get('/zones');

  /// Returns active geofences from the public /geofences endpoint
  Future<Map<String, dynamic>> getGeofences() => get('/geofences');
  ...
  Future<Map<String, dynamic>> getBookingDetail(int id) =>
      get('/bookings/$id');
  Future<Map<String, dynamic>> rateBooking(int bookingId, int rating,
          {String? review}) =>
      post('/bookings/rate', {
  ...
  Future<Map<String, dynamic>> rescheduleBooking(
          int bookingId, String newDate, {String? newTime}) =>
      post('/payments/reschedule', {
        'booking_id': bookingId,
        'new_date': newDate,
  ...
  Future<Map<String, dynamic>> selectSubscriptionDates(
          int id, List<String> dates) =>
  ...
  Future<Map<String, dynamic>> identifyPlant(File image) =>
      _multipart('POST', '/plants/identify', {},
          file: image, fieldName: 'image');
  Future<Map<String, dynamic>> getPlantHistory() =>
      get('/plants/history');

  // ── Shop ──────────────────────────────────────────────────────────────────
  Future<Map<String, dynamic>> getShopCategories() =>
      get('/shop/categories');

  /// Backend expects ?category=<slug/name> NOT category_id
  ...
  // ── Blogs ─────────────────────────────────────────────────────────────────
  Future<Map<String, dynamic>> getBlogs({int page = 1}) =>
      get('/blogs?page=$page');
  Future<Map<String, dynamic>> getBlogBySlug(String slug) =>
      get('/blogs/$slug');
}
